...
class skh_ET_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
					...
					<?php if ( ! in_array( $post_format, array( 'link', 'audio' ) ) || post_password_required( $post ) ) { ?>
						...
						?>
							<button class="skh-post-meta-share" post-id="<?= get_the_ID(); ?>">
								<img alt="<?php _e('Share', "stackhouse") ?>" src="/wp-content/themes/divi-child/includes/builder/module/assets/img/share-icon.png" aria-label="apri finestra esterna di condivisione"/>
							</button>
						</div>
